...
const ShoppingCart = () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.patch(
          `http://localhost:5000/orders/status/${cartItems[0].orderID}`,
          { status: "pending" }
        );
        if (response.status === 200) {
          setLoading(false);
  ...
  useEffect(() => {
    fetchCartItems();
    setCartItemNames(
      cartItems.map((item) => item.name).join(", ") + " are they compatible"
    );
  }, []);
  ...
  const removeItem = async (id) => {
    ...
    const response = await axios.delete(
      `http://localhost:5000/cartproducts/delete/${id}`
    );
    if (response.status === 200) {
      fetchCartItems();
            ...
            <div className="card">
              ...
              <div className="card-body">
                    ...
                    <tbody>
                      {cartItems.map((item) => (
                        <tr key={item.cartID}>
                          <td className="p-4">
                            <div className="media align-items-center d-flex">
                              <img
                                ...
                                className=" imagecartclass d-block ui-w-20 ui-bordered mr-4"
                                alt=""
                              />
                          ...
                          <td className="text-right font-weight-semibold align-middle p-4">
                            ${parseInt(item.price, 10) * item.cartQuantity}
                          </td>
                ...
                <div className="float-right buttonsCart">
                  <button
                    type="button"
                    className="btn  btn-success cartbuttons"
                  ...
                  <button
                    type="button"
                    className="btn  btn-primary cartbuttons"
